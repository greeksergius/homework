...
Для удобства работы и просмотра инфомрации, план инфраструктуры поделен на 5 манифестов terraform:
1. main.tf - основной манифест, в нем инициализируется провайдер Yandex.Сloud. Так как токен является важным элементом безопасности инфраструктуры, он задается в объявленной переменной var.tokenvar. Далее в данном файле описывается конфигурация необходимым виртуальных машин (два веб-сервера nginx, kibana, elasticsearch, grafana, prometheus, bastion), создание резервного копирования. В нем попробовал применить циклы для создания двух ВМ веб-серверов.
...
4. ansibleinventory.tf - сюда вынесено описание настроек для создания инвентарного файла используемого для ansible
5. outputs.tf - описание выдаваемых результатов. Для нас это внешний адрес балансировщика для просмотра веб-сайта и адрес бастиона для подключения по ssh.
